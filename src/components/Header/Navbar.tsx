...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <nav className="hidden lg:block">
          <ul className="flex">
            {menuItems.map((item, index) => (
              <li
                key={index}
                className="w-full whitespace-nowrap group lg:py-3 lg:px-6 lg:border-b border-white border-opacity-0 hover:border-opacity-100 transition-all duration-500 cursor-pointer !border-none"
                onClick={() =>
                  item.subItems
                    ? setActiveSubmenu(activeSubmenu ? null : item.label)
                    : undefined
                }
              >
                {item.subItems ? (
                  <>
                    ...
                    <span
                      className={`lg:absolute lg:top-full lg:bg-white lg:w-[240px] relative shadow-menu lg:text-blue-text rounded-xs text-black ${
                    ...
                    >
                      <ul className="flex flex-col relative lg:-mt-[1px] max-lg:mt-6">
                        {item.subItems.map((subItem, subIndex) => (
                          <li
                            key={subIndex}
                            className="lg:border-t border-blue-light whitespace-nowrap first:border-none"
                          >
                            <Link
            ...
            <div className="ml-6 my-auto">
              <div className="relative lg:block hidden">
                <button
                  ...
                  className="rounded-[132px] flex items-center justify-center space-x-[8px] border px-[14px] py-[8px] lg:text-body-p1 text-res-body-p0 text-white border-white"
                >
                  <svg
                    width="12"
                    height="13"
                  ...
                  <svg
                    className={`${
                      showLanguage ? "rotate-0" : "-rotate-180"
                ...
                <ul
                  className={`absolute overflow-visible bottom-0 translate-y-full bg-white rounded text-black w-max p-3 divide-y-2 rounded-xs shadow-menu ${
              ...
              <div className="lg:hidden block w-full">
                <div className="w-full border px-[14px] py-[8px] text-res-body-p1 text-white border-blue-main transition-all duration-75 rounded-[130px]">
      ...
      <nav
        className={`fixed top-[85px] bg-[#1B76E9] lg:hidden transition-transform inset-x-0 w-full py-8 px-[1.938rem] ${
      ...
      >
        <ul className="flex items-center flex-col space-y-8">
            ...
            >
              {item.subItems ? (
                <>
                  ...
                  <span
                    className={`lg:absolute lg:top-full lg:bg-white lg:w-[240px] relative shadow-menu lg:text-blue-text rounded-xs lg:group-hover:block ${
          ...
          <div className="ml-6 my-auto">
            <div className="lg:hidden block w-full">
              <div
                className={`w-full border px-[14px] py-[8px] text-res-body-p1 text-white border-blue-main transition-all duration-75 ${
                  showLanguage ? "rounded-[20px]" : "rounded-[130px]"
                }`}
              >
                <button
                  onClick={() => setShowLanguage((prev) => !prev)}
                  className="flex justify-center items-center space-x-[8px] w-full"
                >
                  <div className="flex-1 flex space-x-2 items-center">
                    <svg
                      width="12"
                ...
                {showLanguage && (
                  <ul className="text-white w-full text-res-body-p1 mt-2 ml-5">
                    <li className="py-1 hover:text-blue-main hover:transition-colors duration-200">
                      <button className="uppercase">English</button>{" "}
                    </li>
                    <li className="py-1 hover:text-blue-main hover:transition-colors duration-200">
                      <button className="uppercase">Thai</button>{" "}
                    </li>
                    <li className="py-1 hover:text-blue-main hover:transition-colors duration-200">
                      <button className="uppercase">Bahasa Indonesia</button>{" "}
                    </li>
                    <li className="py-1 hover:text-blue-main hover:transition-colors duration-200">
                      <button className="uppercase">Traditional Chinese</button>{" "}
                    </li>
                  </ul>
                )}
              </div>
        ...
        <Link
          href="/en/contact-us"
          className="border border-white bg-transparent hidden max-lg:w-full max-lg:justify-center max-lg:mt-16 pl-6 pr-10 py-[13px] text-res-link lg:text-link lg:flex items-center group/highlight transition-all drop-shadow-button-primary hover:shadow-button-primary rounded-xs !flex"
        >
          Contact Us{" "}
          <svg
